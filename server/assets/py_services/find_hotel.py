...
def get_shelters_by_amenities(airbnb_nbr, range):
    ...
    amn_data['amn_score'] = amn_data[strRangeList].sum(axis=1)
...
def get_shelter_suggestions(nbr, priorities):
    ...
    # rank filtered listings based on ratings
    airbnb_rated = airbnb_nbr_amn.nlargest(5, ['review_scores_rating', 'number_of_reviews_ltm'])




    dists = get_dist_to_shelter(amn_data, airbnb_nbr)
    amn_data['dist'] = dists
