...
neighbourhood_options = neighbourhood_data['neighbourhood'].to_numpy()
...
def input_neighbourhood():
    pick = input('Please pick your highest priority from the following options:\n - {}\n\n'.format('\n - '.join(neighbourhood_options)))
    try:
        pick = get_close_matches(pick, neighbourhood_options, n=1, cutoff=0.6)[0] # Typo correction
    except IndexError:
        pick = None

    while pick not in neighbourhood_options:
        pick = input('\nThat was not an option. Please pick your highest priority from the following options:\n - {}\n\n'.format('\n - '.join(neighbourhood_options)))
        try:
            pick = get_close_matches(pick, neighbourhood_options, n=1, cutoff=0.6)[0] # Typo correction
        except IndexError:
